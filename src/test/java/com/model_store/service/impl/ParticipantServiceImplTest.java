...
class ParticipantServiceImplTest {
    ...
    void setUp() {
        ...
        participant.setCreatedAt(LocalDateTime.now().minusYears(1)); // For experience calculation

        userInfoDto = new UserInfoDto(1L, "Test User", "image.jpg");
        
        // Initialize other DTOs and request objects as needed for tests
        fullParticipantDto = new FullParticipantDto(); // Populate with some defaults
        fullParticipantDto.setId(1L);
        fullParticipantDto.setFullName("Test User");

        findParticipantByLoginDto = new FindParticipantByLoginDto(1L, "Test User", "test@example.com", "image.jpg");

        createParticipantRequest = new CreateParticipantRequest();
        createParticipantRequest.setLogin("newuser");
        createParticipantRequest.setMail("new@example.com");
        createParticipantRequest.setPassword("password");
        createParticipantRequest.setFullName("New User");
    }
    ...
    void findShortInfo_shouldReturnUserInfoDtoWithNullImage_whenNoMainImage() {
        ...
        when(participantMapper.toUserInfoDto(participant, null)).thenReturn(new UserInfoDto(participant.getId(), participant.getFullName(), null));
    ...
    void findActualById_shouldReturnFullParticipantDto_whenDataExists() {
        ...
        when(participantMapper.toFullParticipantDto(participant, addresses, accounts, transfers, imageIds, rating))
        ...
        verify(participantMapper).toFullParticipantDto(participant, addresses, accounts, transfers, imageIds, rating);
    }
    ...
    void findActualById_shouldReturnDtoWithEmptyListsAndDefaultRating_whenNoRelatedData() {
        ...
        when(participantMapper.toFullParticipantDto(eq(participant), eq(Collections.emptyList()), eq(Collections.emptyList()), eq(Collections.emptyList()), eq(imageIds), any(SellerRating.class)))
    ...
    void findByLogin_shouldReturnDtoWithDefaultImageId_whenNoImage() {
        ...
        when(participantMapper.toFindParticipantByLoginDto(participant, 0L )) // 0L is from .defaultIfEmpty(0L)
            .thenReturn(new FindParticipantByLoginDto(participant.getId(), participant.getFullName(), participant.getMail(), null));
    ...
    void findByParams_shouldReturnEnrichedParticipants() {
        ...
        FindParticipantsDto findDto = new FindParticipantsDto();
        findDto.setId(participant.getId());
        ...
        StepVerifier.create(participantService.findByParams(request))
                .assertNext(dto -> {
                    ...
                    assertTrue(dto.getTransferMoneys().contains(TransferMoneyType.SBP));
    ...
    void updateParticipant_shouldUpdateParticipant_whenExistsAndActive() {
        ...
        existingParticipant.setRole("USER"); // Keep original role, mail, password etc.
        existingParticipant.setMail("test@example.com");
        existingParticipant.setPassword("encodedPassword");
        existingParticipant.setCreatedAt(LocalDateTime.now());
        existingParticipant.setSellerStatus(true);
        ...
        mappedParticipant.setPhone(updateRequest.getPhone());
        ...
        finalSavedParticipant.setPhone(updateRequest.getPhone());
        ...
        when(participantRepository.save(any(Participant.class))).thenAnswer(invocation -> {
            ...
            if (p.getId().equals(participantId) &&
                ...
                p.getPhone().equals(updateRequest.getPhone()) &&
