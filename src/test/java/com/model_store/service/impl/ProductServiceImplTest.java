...
class ProductServiceImplTest {
    ...
    void getProductById_shouldReturnProductResponse_whenProductAndCategoryExist() {
        ...
        when(categoryService.findById(product1.getCategoryId())).thenReturn(Mono.just(categoryDto1));
        when(productMapper.toGetProductResponse(product1, categoryDto1, imageIds)).thenReturn(getProductResponse1);
        ...
        verify(categoryService).findById(product1.getCategoryId());
        verify(productMapper).toGetProductResponse(product1, categoryDto1, imageIds);
    }
    ...
    void getProductById_shouldReturnEmpty_whenProductNotFound() {
        ...
        verify(productMapper, never()).toGetProductResponse(any(), any(), any());
    }
    ...
    void getProductById_shouldReturnEmpty_whenCategoryNotFound() {
        ...
        when(productRepository.findActualProduct(productId)).thenReturn(Mono.just(product1));
        when(imageService.findActualImages(productId, ImageTag.PRODUCT)).thenReturn(Flux.fromIterable(imageIds));
        when(categoryService.findById(product1.getCategoryId())).thenReturn(Mono.empty()); // Category not found
    ...
    void getProductById_shouldHandleEmptyImageList() {
        ...
        when(categoryService.findById(product1.getCategoryId())).thenReturn(Mono.just(categoryDto1));
        when(productMapper.toGetProductResponse(product1, categoryDto1, emptyImageIds)).thenReturn(getProductResponse1);
    ...
    void findByParams_shouldReturnPagedProducts() {
        ...
        request.setPageable(new Pageable(0, 10));
        ...
        PagedResult<ProductDto> expectedResult = new PagedResult<>(productDtos, totalCount, request.getPageable());
    ...
    void findByParams_shouldHandleEmptyResultFromRepository() {
        ...
        PagedResult<ProductDto> expectedResult = new PagedResult<>(Collections.emptyList(), 0, request.getPageable());
    ...
    void findByParams_shouldHandleCategoryOrImageNotFoundForAProduct() {
        ...
        when(categoryService.findById(product2.getCategoryId())).thenReturn(Mono.just(categoryDto2));
        ...
        PagedResult<ProductDto> expectedResult = new PagedResult<>(List.of(productDto1, productDto2_noImage), 2, request.getPageable());
    ...
    void createProduct_shouldSaveProductAndUpdateImages() {
        ...
        when(productMapper.toProduct(createRequest, participantId)).thenReturn(productToSave);
        ...
        StepVerifier.create(productService.createProduct(createRequest, participantId))
                .expectNext(productToSave.getId())
                .verifyComplete();

        verify(productMapper).toProduct(createRequest, participantId);
